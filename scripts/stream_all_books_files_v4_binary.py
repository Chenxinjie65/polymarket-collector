...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "all_market_meta.json + <market>/timestamps.bin + assetN.bids.bin + assetN.asks.bin."
        )
    )
    parser.add_argument("--data-root", default="data_all_books_files_v4_live", help="Output root")
    parser.add_argument(
        "--duration-seconds",
        type=int,
        default=1800,
...
def scale_decimal(value: Any, scale: int) -> int:
    if value in (None, ""):
        return 0
    try:
...
def encode_timestamp_record(timestamp_ms: int) -> bytes:
    return struct.pack("<Q", max(0, timestamp_ms))


def encode_levels_record(levels: Any) -> bytes:
    ...
    for level in levels:
        ...
        if price in (None, "") or size in (None, ""):
            continue
        scaled_price = scale_decimal(price, PRICE_SCALE)
        scaled_size = scale_decimal(size, SIZE_SCALE)
        if scaled_price > 0xFFFFFFFF or scaled_size > 0xFFFFFFFF:
            continue
        pairs.append(scaled_price)
        pairs.append(scaled_size)

    count = len(pairs) // 2
    if count == 0:
        return struct.pack("<H", 0)

    record = bytearray(struct.pack("<H", count))
    for index in range(0, len(pairs), 2):
        record.extend(struct.pack("<II", pairs[index], pairs[index + 1]))
    return bytes(record)
...
def fetch_markets_and_assets(*, page_limit: int) -> tuple[dict[str, MarketMeta], list[str]]:
...
async def fetch_markets_and_assets_with_retry(
    *, page_limit: int, stop_at: float | None
) -> tuple[dict[str, MarketMeta], list[str]]:
    failure_streak = 0
    while True:
        try:
            return await asyncio.to_thread(fetch_markets_and_assets, page_limit=page_limit)
...
def write_all_market_meta(meta_path: Path, market_catalog: dict[str, MarketMeta]) -> None:
    ...
    payload = {
        ...
        "timestamp_format": "repeated(uint64_le ts_event_ms)",
        "book_format": "repeated(uint16_le level_count, repeated(uint32_le price_scaled, uint32_le size_scaled))",
        "markets": [
            {
...
def count_market_rows(data_root: Path, market_id: str) -> int:
    ts_path = market_dir(data_root, market_id) / "timestamps.bin"
    if not ts_path.exists():
        return 0
    return ts_path.stat().st_size // 8


def write_zero_rows(path: Path, row_count: int) -> None:
    if row_count <= 0:
    ...
    with path.open("wb") as f:
        f.write(struct.pack("<H", 0) * row_count)


async def backfill_new_asset_rows(
    *,
    data_root: Path,
    market_id: str,
    asset_index: int,
) -> None:
    row_count = await asyncio.to_thread(count_market_rows, data_root, market_id)
    mdir = market_dir(data_root, market_id)
    bids_path = mdir / f"asset{asset_index}.bids.bin"
    asks_path = mdir / f"asset{asset_index}.asks.bin"
    await asyncio.to_thread(write_zero_rows, bids_path, row_count)
    await asyncio.to_thread(write_zero_rows, asks_path, row_count)
    ...
    if existing is None:
        ...
        for asset in market_meta.assets:
            ...
            if current is not None:
                ...
                await backfill_new_asset_rows(
                    data_root=state.data_root,
                    market_id=market_meta.market_id,
                    asset_index=new_asset.asset_index,
                )
    ...
    while True:
        ...
        for book in books:
            ...
            if market_id in state.resolved_markets or market_id not in state.market_catalog:
                continue
            grouped.setdefault(market_id, []).append(book)

        for market_id, market_books in grouped.items():
            ...
            books_by_timestamp: dict[int, dict[str, dict[str, Any]]] = {}
            for book in market_books:
                asset_id = str(book.get("asset_id") or "")
                if asset_id not in asset_by_id:
                    continue
                try:
                    ts_event = int(str(book.get("timestamp") or "0"))
                except ValueError:
                    ts_event = 0
                books_by_timestamp.setdefault(ts_event, {})[asset_id] = book

            if not books_by_timestamp:
                continue

            timestamp_records = bytearray()
            bids_records_by_asset: dict[int, bytearray] = {
                asset.asset_index: bytearray() for asset in market_meta.assets
            }
            asks_records_by_asset: dict[int, bytearray] = {
                asset.asset_index: bytearray() for asset in market_meta.assets
            }

            for ts_event in sorted(books_by_timestamp):
                timestamp_records.extend(encode_timestamp_record(ts_event))
                row_books = books_by_timestamp[ts_event]
                for asset in market_meta.assets:
                    book = row_books.get(asset.asset_id)
                    bids_records_by_asset[asset.asset_index].extend(
                        encode_levels_record(book.get("bids") if book else None)
                    )
                    asks_records_by_asset[asset.asset_index].extend(
                        encode_levels_record(book.get("asks") if book else None)
                    )

            with (mdir / "timestamps.bin").open("ab") as f:
                f.write(timestamp_records)
            stats.timestamp_bytes += len(timestamp_records)

            for asset in market_meta.assets:
                bids_path = mdir / f"asset{asset.asset_index}.bids.bin"
                asks_path = mdir / f"asset{asset.asset_index}.asks.bin"

                bids_blob = bids_records_by_asset[asset.asset_index]
                asks_blob = asks_records_by_asset[asset.asset_index]

                with bids_path.open("ab") as f:
                    f.write(bids_blob)
                stats.bids_bytes += len(bids_blob)

                with asks_path.open("ab") as f:
                    f.write(asks_blob)
                stats.asks_bytes += len(asks_blob)
        ...
        try:
            async with connect(
                ...
                while stop_at is None or asyncio.get_running_loop().time() < stop_at:
                    ...
                    books = normalize_books_payload(payload)
                    if not books:
                        continue

                    await queue.put(books)
                    stats.ws_messages += 1
                    stats.ws_book_entries += len(books)
                    last_books_at = asyncio.get_running_loop().time()
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    market_catalog, asset_ids = await fetch_markets_and_assets_with_retry(page_limit=args.page_limit, stop_at=stop_at)
    ...
    payload = {
        ...
        "timestamp_bytes": stats.timestamp_bytes,
        "bids_bytes": stats.bids_bytes,
        "asks_bytes": stats.asks_bytes,
